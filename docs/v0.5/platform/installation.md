...
oc apply -f deploy/v0.5/operator.yaml

# 3. Create your namespace (or use existing)
oc create namespace <your-namespace>

# 4. Configure and apply secrets
cp deploy/v0.5/secrets.yaml.template deploy/v0.5/secrets.yaml
oc apply -n <your-namespace> -f deploy/v0.5/secrets.yaml

# 5. Deploy application resources
oc apply -n <your-namespace> -f deploy/v0.5/app.yaml

# Edit deploy/v0.5/secrets.yaml with your actual credentials
oc apply -n <your-namespace> -f deploy/v0.5/secrets.yaml
...
All deployment files are located in the `deploy/v0.5/` directory at the root of the repository.

### 1. Operator Installation

**File:** `deploy/v0.5/operator.yaml`

This installs the Red Hat Developer Hub operator. This is cluster-scoped and only needs to be installed once.

```yaml
{% include deploy-v0.5/operator.yaml %}
```
...
```bash
oc apply -n my-custom-namespace -f deploy/v0.5/app.yaml
```

### Plugin Versions

To use different plugin versions, update the OCI image references in the `dynamic-plugins` ConfigMap section of `deploy/v0.5/app.yaml`. The default manifest also includes the MCP server, X2A MCP extras, and the upstream `@backstage/plugin-auth` DCR consent UI alongside the Conversion Hub plugins.

## MCP tools

The default [`deploy/v0.5/app.yaml`]({% link v0.5/platform/installation.md %}#2-application-deployment) wires up Model Context Protocol (MCP) so assistants can call X2A MCP tools against your RHDH X2A route.

Use [MCP tools]({% link v0.5/platform/mcp-server.md %}) for the tool list and permissions description.
...
YAML examples and behavior notes for those keys live on the [MCP tools]({% link v0.5/platform/mcp-server.md %}#advanced-configuration) page.

After any change to `deploy/v0.5/app.yaml`, re-apply and restart the RHDH pod so configuration and dynamic plugins reload:
