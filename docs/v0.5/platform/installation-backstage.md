...
This guide adds the X2A plugins to a stock Backstage app created with the upstream wizard, not Red Hat Developer Hub on OpenShift. For RHDH deployment, use [Installation]({% link v0.5/platform/installation.md %}) instead.
...
Core X2A conversion flows do not require the packages below. Add them only when you need DCR consent UI and/or MCP tool wiring similar to the production RHDH overlays in [`deploy/v0.5/app.yaml`](https://github.com/x2ansible/x2ansible.github.io/blob/main/deploy/v0.5/app.yaml).
...
Enable DCR under `auth.experimentalDynamicClientRegistration` in `app-config.yaml`. For a full example, see [`deploy/v0.5/app.yaml`](https://github.com/x2ansible/x2ansible.github.io/blob/main/deploy/v0.5/app.yaml).
...
For `mcpActions` and related `app-config` fragments, see [MCP tools - Advanced configuration]({% link v0.5/platform/mcp-server.md %}#advanced-configuration).
...
CSV-driven bulk flows and the `RepoAuthentication` extension are described in [CSV Bulk Import]({% link v0.5/platform/csv-bulk-import.md %}).

## Configuration (pointers only)

| Topic                                                            | Where it is documented                                                                                                                                                                                                                                          |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OAuth providers, env vars, sign-in                               | [Authentication]({% link v0.5/platform/authentication.md %})                                                                                                                                                                                                         |
| RBAC / permissions for the `x2a` plugin                          | [Authorization]({% link v0.5/platform/authorization.md %})                                                                                                                                                                                                           |
...
If the backend API base URL seen by clients or integrations is not the default, you may need `x2a.callbackBaseUrl` (see in-cluster example in [deploy/v0.5/app.yaml](https://github.com/x2ansible/x2ansible.github.io/blob/main/deploy/v0.5/app.yaml)); local `yarn start` often works without it.
...
Use [API Reference]({% link v0.5/platform/api-reference.md %}) for the REST surface once the backend plugin is running.
