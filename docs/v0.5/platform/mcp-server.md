...
With the default [`deploy/v0.5/app.yaml`]({% link v0.5/platform/installation.md %}#2-application-deployment), X2A registers MCP tools so assistants (for example Cursor, Continue, or other tool-aware clients) can work with migration projects through the same RHDH X2A instance you use in the browser.
...
Get your RHDH base URL from the cluster after [Installation]({% link v0.5/platform/installation.md %}#access-the-application).
...
That subject must be granted the same [Authorization]({% link v0.5/platform/authorization.md %}) roles as a human user for the actions you want.
...
See [Authorization]({% link v0.5/platform/authorization.md %}) for `x2a.admin` and `x2a.user` and how to assign them to users, groups, or an MCP service subject.

## Advanced configuration
{: #advanced-configuration}

The following fragments belong in the **single** `app-config-rhdh.yaml` document carried by the `app-config-rhdh` ConfigMap (see [Installation]({% link v0.5/platform/installation.md %})). Keep **one** top-level `backend:` map and merge new keys into it instead of duplicating the key.
...
- [Installation]({% link v0.5/platform/installation.md %}): deploy manifests, secrets, and restarting the RHDH pod
